...
// Roles mapping
const users = {
  admin: { email: 'admin@hastag.com', expectedUrl: '/admin/dashboard' },
  employee: { email: 'sakshi@hastag.com', expectedUrl: '/employee/dashboard' },
  sales: { email: 'sales1@hastag.com', expectedUrl: '/sales/dashboard' },
  client: { email: 'adity@hastag.com', expectedUrl: '/client/dashboard' },
};
...
test.describe('Thorough E2E Feature Testing', () => {
  ...
  test('Employee Path: Login, Add Task, Toggle Task, Start Session', async ({ browser }) => {
    ...
    await page.waitForSelector('text=My Tasks', { timeout: 10000 });
    ...
    // 4. Session Start
    await page.goto('http://localhost:8080/employee/dashboard');
  ...
  test('Admin Path: Basic Health Check', async ({ browser }) => {
    ...
    await expect(page.locator('text=Employee Operations').first()).toBeVisible({ timeout: 15000 });
